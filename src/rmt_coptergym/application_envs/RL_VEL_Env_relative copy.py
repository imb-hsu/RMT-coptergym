...
class VEL_Env(RMT_RL_Env):
    ...
    def _compute_reward_terms(self) -> float:
        ...
        #not used 0.5*float(np.mean(self._calculate_reward_from_error(norm_action_delta)))+0.5* --> flawed code
        # --- 4. NEU: Paired Efficiency Reward (Dein physikalischer Ansatz) ---
        pair_error = np.abs(self.action.cmd[[1,0,2,3]] - self.action.cmd[[4,5,7,6]]) / self.limits.motor.range
        ...
        lower_bound = self.limits.motor.min + 0.35 * self.limits.motor.range # bei 25%
        upper_bound = self.limits.motor.min + 0.65 * self.limits.motor.range # bei 75%
        ...
        action_delta = self.action.cmd - self.action.cmd_old
        # Normalisiere den Fehler. Die maximale Änderung pro Schritt ist theoretisch die
        # gesamte Motor-Range. Wir nehmen die halbe Range als realistische Skalierung.
        # np.linalg.norm fasst die Änderung aller 8 Motoren in einer Zahl zusammen.
        smoothness_error = np.linalg.norm(action_delta) / (50+15)
        ...
        reward_components = np.array([
            ...
            comfort_zone_reward ** weights.get('comfort_zone', 1.0),
        ...
        base_reward = float(np.exp(np.mean(log_rewards)))
        ...
        self.reward.terms['omega']              = {'w': weights.get('omega', 1.0),              'r': yaw_reward, 'error': self.normalized_dict['omega']}
        self.reward.terms['accel']              = {'w': weights.get('accel', 1.0),              'r': yaw_reward, 'error': self.normalized_dict['accel']}
