...
class VEL_Env(RMT_RL_Env):
    ...
    def __init__(self, 
        ...
        self.integral_limit_vel = 10*self.goal.tolerance.vel # m/s (Passend zur Scale 'int')
    ...
    def _compute_reward_terms(self) -> float:
        ...
        # Integral setup: (minor leaking)
        new_error = np.concatenate([self.error.vel_c, self.error.rpy_deg])
        self.integral_error = 0.999 * self.integral_error + new_error * self.dt
        ...
        self.integral_error_norm = np.concatenate([self.integral_error[0:3]/self.integral_limit_vel, self.integral_error[3:6]/self.integral_limit_rpy_deg])
        ...
        if self.reward_type == "multiplicative":
            reward_components = np.array([
                integral_reward     ** weights.get('integral', 1.0),
        ...
        base_reward = 0.0 # in case of Issue easy to debug
        ...
        if self.reward_type == "additive":
            terms = [
                    (weights.get('integral', 1.0),          integral_reward),
        ...
        self.reward.terms['integral']           = {'w': weights.get('integral', 1.0),           'r': integral_reward, 'error': self.integral_error_norm}
