...
void xzlascl_6I4v9nni(real_T cfrom, real_T cto, int32_T m, real_T A_data[])
{
  real_T cfrom1;
  real_T cfromc;
  real_T cto1;
  ...
  while (notdone) {
    ...
    b = (uint8_T)m;
    for (b_i = 0; b_i < b; b_i = b_i + 1) {
      A_data[(b_i + 1) - 1] = A_data[(b_i + 1) - 1] * mul;
    }
  }
}
