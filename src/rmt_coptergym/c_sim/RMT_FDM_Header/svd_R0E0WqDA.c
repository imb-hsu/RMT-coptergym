...
#include "xnrm2_MOAbR545.h"
#include <math.h>
#include "xnrm2_AEVfnlB5.h"
...
void svd_R0E0WqDA(const real_T A[16], real_T U[16], real_T s[4], real_T V[16])
{
  real_T Vf[16];
  real_T b_A[16];
  real_T b_s[4];
  ...
  for (b_q = 0; b_q < 3; b_q = b_q + 1) {
    ...
    nrm = xnrm2_MOAbR545((4 - (b_q + 1)) + 1, b_A, qq);
    if (nrm > 0.0) {
    ...
    if (b_q + 1 <= 2) {
      nrm = xnrm2_AEVfnlB5(4 - (b_q + 1), e, (b_q + 1) + 1);
      if (nrm == 0.0) {
        e[(b_q + 1) - 1] = 0.0;
      } else {
        if (e[((b_q + 1) + 1) - 1] < 0.0) {
