...
void svd_UQNWgDyT(const real_T A[60], real_T U[60], real_T s[6], real_T V[36])
{
  real_T b_A[60];
  real_T Vf[36];
  real_T work[10];
  ...
  for (b_q = 0; b_q < 6; b_q = b_q + 1) {
    ...
    nrm = xnrm2_TWLjRXVi((10 - (b_q + 1)) + 1, b_A, qq);
    if (nrm > 0.0) {
    ...
    if (b_q + 1 <= 4) {
      nrm = xnrm2_ljsAr6fY(6 - (b_q + 1), e, (b_q + 1) + 1);
      if (nrm == 0.0) {
        e[(b_q + 1) - 1] = 0.0;
      } else {
        if (e[((b_q + 1) + 1) - 1] < 0.0) {
