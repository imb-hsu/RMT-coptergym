...
#include "xnrm2_cbvgxlmq.h"
#include "xnrm2_fIcS3nDs.h"
...
void svd_YCkpH4GJ(const real_T A[60], real_T U[36], real_T s[6], real_T V[100])
{
  real_T b_A[60];
  real_T e[10];
  real_T b_s[7];
  ...
  for (b_q = 0; b_q < 6; b_q = b_q + 1) {
    ...
    if (b_q + 1 <= 5) {
      nrm = xnrm2_cbvgxlmq((6 - (b_q + 1)) + 1, b_A, qq);
      if (nrm > 0.0) {
    ...
    nrm = xnrm2_fIcS3nDs(10 - (b_q + 1), e, (b_q + 1) + 1);
    if (nrm == 0.0) {
      e[(b_q + 1) - 1] = 0.0;
    } else {
      if (e[((b_q + 1) + 1) - 1] < 0.0) {
