...
#include "xnrm2_WjwGuFZ9.h"
#include "xnrm2_WPsymYPi.h"
...
void svd_uqcoX62M(const real_T A_data[], const int32_T A_size[2], real_T U_data[],
                  int32_T *U_size)
{
  real_T b_A_data[100];
  real_T e_data[10];
  ...
  for (b_q = 0; b_q < h; b_q = b_q + 1) {
    ...
    if (b_q + 1 <= nct) {
      nrm = xnrm2_WjwGuFZ9((10 - (b_q + 1)) + 1, b_A_data, qq);
      if (nrm > 0.0) {
    ...
    if (b_q + 1 <= p - 2) {
      pmq = p - (b_q + 1);
      nrm = xnrm2_WPsymYPi(pmq, e_data, (b_q + 1) + 1);
      if (nrm == 0.0) {
        e_data[(b_q + 1) - 1] = 0.0;
      } else {
